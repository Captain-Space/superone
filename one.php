...
class one
{
    ...
    public static function cache_refresh($drives = 'default')
    {
        echo '缓存更新';
    ...
    public static function token_refresh($drives = 'default')
    {
    ...
    public static function upload_file($localfile, $remotefile = null, $drives = 'default')
    {
        $drivesfile = ROOT.'config/'.$drives.'.php';
    ...
    public static function upload_folder($localfolder, $remotefolder = '/')
    {
        $localfolder = realpath($localfolder);
        $remotefolder = get_absolute_path($remotefolder);
        echo ' 开始上传文件夹'.PHP_EOL;
        self::folder2upload($localfolder, $remotefolder);
    }

    public static function folder2upload($localfolder, $remotefolder)
    {
    ...
    public static function upload_large_file($localfile, $remotepath)
    {
        ...
        echo ' 上传分块'.onedrive::human_filesize($info['length']).'	';
        $begin_time = microtime(true);
        ...
        if (!empty($data['nextExpectedRanges'])) {
            ...
            echo onedrive::human_filesize($info['speed']).'/s'.'	'.round(($info['offset'] / $info['filesize']) * 100).'%	'.PHP_EOL;
    ...
    public static function ls($path = '/', $drives = 'default')
    {
        $drivesfile = ROOT.'config/'.$drives.'.php';
